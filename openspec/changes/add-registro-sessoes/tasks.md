...
- [ ] 1.1 Documentar em `docs/formato-sessoes.md` o envelope de `data/sessoes.json`
...
- [ ] 1.2 Adicionar `data/` ao `.gitignore`. Verificação: `git status` não mostra
      `data/sessoes.json` depois de rodar o comando.

## 2. Regras de negócio

- [ ] 2.1 Criar `src/estudo/__init__.py` e `src/estudo/regras.py` com a exceção de domínio
      `ErroDeRegra`. Verificação: `python -c "from src.estudo.regras import ErroDeRegra"`
      roda sem erro.
- [ ] 2.2 Implementar a validação de tópico, duração (5 a 480, limites aceitos), data no
      futuro e término no mesmo dia, cada uma com mensagem própria. Verificação: os testes
      da tarefa 3.1 passam.
- [ ] 2.3 Implementar a detecção de conflito usando intervalo semiaberto e a atribuição do
      próximo id, sem tocar em disco. Verificação: os testes da tarefa 3.2 passam.

## 3. Testes das regras

- [ ] 3.1 Escrever `tests/test_regras.py` cobrindo os cinco motivos de recusa e o caminho
      feliz. Verificação: `python -m pytest tests/test_regras.py -q` passa.
- [ ] 3.2 Acrescentar os testes de conflito: sobreposição parcial, sessões encostadas (caso
...
- [ ] 4.1 Implementar `src/estudo/armazenamento.py` com leitura (arquivo ausente devolve
...
- [ ] 5.1 Implementar `src/estudo/cli.py` com `argparse` para o subcomando `add`
      ...
      imprime o id.
- [ ] 5.2 Mapear `ErroDeRegra` para mensagem em `stderr` e código de saída 1, sem stack
      trace. Verificação: `echo $?` devolve 1 depois de um comando recusado.

## 6. Testes ponta a ponta

- [ ] 6.1 Escrever `tests/test_cli_add.py` usando um arquivo temporário, cobrindo os cinco
...
- [ ] 7.1 Atualizar `README.md` e `CLAUDE.md` com o comando `estudo add`, a faixa de duração
